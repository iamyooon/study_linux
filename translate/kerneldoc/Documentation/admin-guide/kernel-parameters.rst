...
다음은 __setup(), core_param() 및 module_param() 매크로에 의해 구현되고 영어 사전 순서(모든 구두점을 무시하고 대소 문자를 구분하지 않고 글자 앞에 숫자 정렬)로 정렬된 알려진 설명이 함께 있는 커널 파라미터의 전체목록이다.
...
커널은 커널 커맨드라인의 파라미터를 "--" 까지 파싱한다. 파라미터가 '.'을 포함하지 않으면 인식되지 않고 파라미터는 init에 전달된다. '='파라미터는 init 환경으로 이동하고 다른 파라미터는 init에 커맨드라인 인수로 전달됩니다. "--" 다음의 모든 것은 init의 인수로 전달된다.
The kernel parses parameters from the kernel command line up to "--"; if it doesn't recognize a parameter and it doesn't contain a '.', the parameter gets passed to init: parameters with '=' go into init's environment, others are passed as command line arguments to init.  Everything after "--" is passed as an argument to init.

모듈 파라미터는 두 가지 방법으로 지정할 수 있다. 모듈 이름 접두사가있는 커널 커맨드라인을 사용하거나 modprobe를 통해 지정할 수 있다. 아래가 한 예다.
...
커널에 내장 된 모듈의 파라미터는 커널 커맨드라인에 지정해야 한다. modprobe는 커널 커맨드라인 (/proc/cmdline)을 살펴보고 모듈을 로드 할 때 모듈 파라미터를 수집하므로 커널 커맨드라인을 로드 가능한 모듈에도 사용할 수 있다.
...
파라미터 안의 하이픈(-)과 밑줄(_)은 동일하므로, 다음은 
...
쌍따옴표는 아래와 같이 값안의 공백을 보호할 때 사용할 수 있다.
